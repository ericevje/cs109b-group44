...
def boundary_points(min_point, max_point):
    start_face = []
    end_face = []
...
def main():
    ...
    start_face, end_face = boundary_points(min_point, max_point)
    distance = max_point[0] - min_point[0]
    ...
    r = client.add_extrude(sketch_name, profile_id, distance, "NewBodyFeatureOperation")
    r = client.graph(file='', dir='', format='PerExtrude')
    response_json = r.json()
    print(response_json['data']['graph'].keys())
